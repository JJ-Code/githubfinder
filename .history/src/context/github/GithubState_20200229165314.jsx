...
const GithubState = props => {
  ...
  const searchUsers = async text => {
    setLoading()
    const res = await axios.get(`https://api.github.com/search/users?q=${text}&client_id=${gitHubClientId}&client_secert=${process.env.REACT_APP_GITHUB_CLIENT_SECRET}`)
    //console.log(res);
    dispatch({
      type: SEARCH_USERS,
  ...
  const getUser = async (username) => {
    setLoading();
    const res = await axios.get(`https://api.github.com/users/${username}?client_id=${gitHubClientId}&client_secert=${process.env.REACT_APP_GITHUB_CLIENT_SECRET}`)
    //console.log(res);
    dispatch({
      type: GET_USER,
      payload: res.data
